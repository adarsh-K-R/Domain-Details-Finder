...
const recipientEmail = 'akr1874@gmail.com';
const senderEmail = 'whoistrial@gmail.com';
const senderPassword = 'amovdfosqqdmvtgm';
const service = 'gmail';
...
schedule.scheduleJob('00 10 20 * *', async () => {
  ...
  const newlyRegisteredDomains = fileData.split('\n').map(line => line.trim());
  ...
  for (const domain of newlyRegisteredDomains) {
    ...
    if (!processedDomains.has(domain)) {
      try {
        ...
        errorLogger.error(`Error during WHOIS lookup for ${domain}: ${error.message}`);
      }
    } else {
...
function extractDetails(whoisData) {
  ...
  for (const line of lines) {
    ...
    if (line.includes('Registrant Name:')) {
      ...
      if((details.Name.includes('REDACTED FOR PRIVACY')) || (details.Name.includes('Redacted for privacy')) || (details.Name.includes('Redacted For Privacy')) || (details.Name.includes('Registration Private')) || (details.Name.includes('Contact Privacy')) || (details.Name.includes('Privacy protection')) || (details.Name.includes('Whois Privacy')) || (details.Name.includes('Protected Protected')) || (details.Name.includes('GDPR Masked')) || (details.Name.includes('Domain Admin')) || details.Name.includes('DOMAIN ADMINISTRATOR') || details.Name.includes('DATA REDACTED') || details.Name.includes('Admin Contact') || details.Name.includes('Domain Privacy') || details.Name.includes('Domain ID Shield Service')){
        return null;
      }
    }
